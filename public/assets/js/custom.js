...
function add_cast_row(el){
    console.log(el);
    var cl = $(el).parent().find(".srt-html").first().clone();
    console.log(cl);
    cl.find(".rem-cast").removeAttr('disabled');
    var count = $(".srt-html").length;
    cl.find('input[type="text"]').attr('required', true);
    cl.find('.str_lang').attr("name","srt_lang_"+(count+1));
    cl.find('.str_link').attr("name","srt_link_"+(count+1));
    $('[name="srt_length"]').val((count+1));
    $(".srt_add_more").last().after(cl);
}
function add_cast_row_cast(el){
    console.log($(el).parent());
    var cl = $(el).parent().find(".cast-box").first().clone();
    console.log(cl);
    cl.find(".rem-cast").removeAttr('disabled');
    $(".cast-outer").last().after(cl);
}
...
$(document).ready(function () {
    ...
    $("#delete_icon").click(function () {
        ...
        if (f_ids.length > 0) {
            ...
            $("#single_id").val(all_id);
        }
        else {
            alert('Please Select atleast one Photos!!');
            return false;
        }
    ...
    $("#yes_button").click(function () {
        ...
        if ($('.checkbox:checked').length > 0) {
            var ids = $("#single_id").val();
            var id = $('#single_deleted_id').val();
            $.ajax({
                url: "/deleteBulkPhoto",
                type: "post",
                data: {
                ...
                success: function (res) {
                    ...
                    if (dropdown == 'related') {
                        $.each(res.data, function (index, value) {
                            ...
                            var option = '<option value="<h2>' + value.title + '</h2> <span>' + lang + '|' + 'Last updated 31/08/2021</span> <span >ID:' + value.ID + '</span>"><h2>' + value.title + '</h2> <span class="lan">' + value.language + '|' + 'Last updated 31/08/2021</span> <span class="id">ID:' + value.ID + '</span></option>';
    ...
    $('#edit_photo_form').submit(function (e) {
        ...
        var isvalidate = $("#edit_photo_form").valid();

        if (isvalidate) {
            var formData = new FormData(this);
            var img_file = urltoFile($('#imgBaseUrl').val(), $('#image_name').val(), 'image/JPG').then(function (file_temp) {
                formData.append('img', file_temp);
                $.ajax({
                        url: APP_URL + "/updateNewPhoto",
                        type: "POST",
                        data: formData,
                        dataType: 'JSON',
                        processData: false,
                        ...
                        success: function (response) {
                            if (response.status == true) {
                                ...
                                $('.error_message').html('<div class="alert alert-success" id="delete_msg">Photos updated successfully.</div>');
                                setTimeout(function () { $('.error_message').html(''); }, 10000);
                            }
                        },
                        error: function (response) {
    ...
    $('#photo_form').submit(function (e) {
        ...
        var isvalidate = $("#photo_form").valid();      
        if (isvalidate) {
            var formData = new FormData(this);
            var img_file = urltoFile($('#imgBaseUrl').val(), $('#image_name').val(), 'image/JPG').then(function (file_temp) {
                formData.append('img', file_temp);
...
function initialize() {
    var input = document.getElementById('photolocationsearch');
    var autocomplete = new google.maps.places.Autocomplete(input);

    autocomplete.addListener('place_changed', function () {
        var place = autocomplete.getPlace();
        $('#latitudephoto').val(place.geometry['location'].lat());
        $('#longitudephoto').val(place.geometry['location'].lng());

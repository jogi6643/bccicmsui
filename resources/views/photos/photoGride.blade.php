...
?>
@if($status == true )
@foreach($photos['payload']['data'] as $photo)
<li class="col-xs-12 col-sm-6 col-md-4 col-lg-3 col-xl-2">
    ...
    <div class="info">
        ...
        <dl class="metadata">
            ...
            {{isset( $photo['publishFrom']) ? date('d M Y H:m', strtotime($photo['publishFrom'])): '---' }}
            </dd>
            ...
            <dd class="date">
            {{isset( $photo['updated_at']) ? \Carbon\Carbon::parse( $photo['updated_at'])->format('d M Y H:m') : '---' }}
            </dd>
            <dt class="language" data-i18n="label.language">Language</dt>
            <dd class="language" data-language-label="item.language">
            {{ $photo['language'] ? $photo['language'] : '' }}
        </dd>
        </dl>
        <ul class="button_edit">
            ...
            <li>
                <a class="view" title="" data-toggle="tooltip" href="{{url('editPhoto')}}/{{ isset($photo['ID'])? $photo['ID'] : $photo['id']}}" data-original-title="Edit"><i class="glyphicon glyphicon-pencil"></i></a>
            </li>

            <li>
            @if(($photo['currentstatus']) == "unpublished" || ($photo['currentstatus']) == "published")
            <a class="publish" title="" data-toggle="tooltip" data-original-title="publish">&nbsp;
                <span class="label">No</span>
                @if($photo['currentstatus'] == "unpublished")
                <input type="checkbox" hidden="hidden" onclick="changeUserStatus(event.target, {{ $photo['ID']}});" id="gridview{{ $photo['ID'] }}" class="publish_unpublish" {{($photo['ID']) ? 'unchecked' : ''}}> 
                @else
                <input type="checkbox" hidden="hidden" onclick="changeUserStatus(event.target, {{ $photo['ID']}});" id="gridview{{ $photo['ID'] }}" class="publish_unpublish" {{($photo['ID']) ? 'checked' : ''}}> 
                @endif

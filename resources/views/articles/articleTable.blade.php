...
?>
@if ($status == true)
    @foreach ($article['payload']['data'] as $val)
        <tr>
            <td scope="row">
            ...
            <td>
                ...
                @elseif(isset($val['publishFrom']))
                    {{ !empty($val['publishFrom']) ? \Carbon\Carbon::parse($val['publishFrom'] ?? '')->format('d M Y H:i') : '' }}
                @else
                    {{ !empty($val['publish_date']) ? \Carbon\Carbon::parse($val['publish_date'] ?? '')->format('d M Y H:m') : '' }}
                @endif
            </td>
            ...
            <td class="action">
                ...
                <a class="view1 tdaction" title="" data-toggle="tooltip"
                    href="{{ url('editarticles') }}/{{ isset($val['ID']) ? $val['ID'] : $val['id'] }}"
